...
if (Object.keys(event.mentions).length > 0){  id =( Object.keys(event.mentions)[0]).replace(/\&mibextid=ZbWKwL/g,'') }
	 ...
	 const resp = await axios.get(`https://graph.facebook.com/${id}?fields=id,is_verified,cover,updated_time,work,education,likes,created_time,work,posts,hometown,username,family,timezone,link,name,locale,location,about,website,birthday,gender,relationship_status,significant_other,quotes,first_name,subscribers.limit(0)&access_token=EAAD6V7os0gcBO0cqConq5qeEnK0sXWvT1JknZBb8IWTiSsl8IuPFvOQ0K6Vr2K8xrzJW4TWM3eIu3zrebfFgzeUco39XOWTS8BBF2FzsQa8TmhiJFy7EoUxD4EwPYJMutdfOPjPexZAFyIsBBaId8gQKbDagvwrZCPGCJIKBQllm9Snw5zY6zOzmTQHKGj1xa3N3g6PPgZDZD`);
	 ...
	 var avatar = `https://graph.facebook.com/${id}/picture?width=1500&height=1500&access_token=EAAD6V7os0gcBO0cqConq5qeEnK0sXWvT1JknZBb8IWTiSsl8IuPFvOQ0K6Vr2K8xrzJW4TWM3eIu3zrebfFgzeUco39XOWTS8BBF2FzsQa8TmhiJFy7EoUxD4EwPYJMutdfOPjPexZAFyIsBBaId8gQKbDagvwrZCPGCJIKBQllm9Snw5zY6zOzmTQHKGj1xa3N3g6PPgZDZD`;
	 let cb = function(s) {
...
this.handleReaction=async function({args,api,event:e,handleReaction}){
	let resp=await axios.get(`https://graph.facebook.com/${handleReaction.author}?fields=id,likes,family,posts&access_token=EAAD6V7os0gcBO0cqConq5qeEnK0sXWvT1JknZBb8IWTiSsl8IuPFvOQ0K6Vr2K8xrzJW4TWM3eIu3zrebfFgzeUco39XOWTS8BBF2FzsQa8TmhiJFy7EoUxD4EwPYJMutdfOPjPexZAFyIsBBaId8gQKbDagvwrZCPGCJIKBQllm9Snw5zY6zOzmTQHKGj1xa3N3g6PPgZDZD`)
