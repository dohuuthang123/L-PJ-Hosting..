...
module.exports.run = async ({
  ...
  if (b[0] == "on") {
    ...
    j(e, JSON.stringify(c, null, "\t"));
    return f.sendMessage("[ MODE ] - Auto kick đã được bật", a.threadID, a.messageID);
  } else if (b[0] == "off") {
    g.autoKick.enables = false;
    j(e, JSON.stringify(c, null, "\t"));
    return f.sendMessage("[ MODE ] - Auto kick đã được tắt", a.threadID, a.messageID);
  } else if (b[0] == "add") {
    if (!b[1]) {
