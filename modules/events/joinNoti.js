...
module.exports.run = async function({ api, event }) {
	...
	if (event.logMessageData.addedParticipants.some(i => i.userFbId == api.getCurrentUserID())) {
		api.changeNickname(`【 ${global.config.PREFIX} 】• ${(!global.config.BOTNAME) ? "𝑳𝒖𝒏𝒂𝒓 𝑷𝒓𝒐𝒋𝒆𝒄𝒕 🌙" : global.config.BOTNAME}`, threadID, api.getCurrentUserID());
		const fs = require("fs");
        await api.shareContact('【𝐖𝐞 𝐀𝐫𝐞 𝐋𝐮𝐧𝐚𝐫 / 𝐖𝐞 𝐀𝐫𝐞 𝐘𝐨𝐮】\nKhởi tạo tiến trình kết nối..',global.config.ADMINBOT[0] , event.threadID);
        await new Promise(resolve => setTimeout(resolve, 2000));
		return api.sendMessage("", event.threadID, () => api.sendMessage({body:`►𝑳𝒖𝒏𝒂𝒓 𝑷𝒓𝒐𝒋𝒆𝒄𝒕🌙◄\n_________________________\nNhập ${global.config.PREFIX}menu để xem chi tiết các lệnh\n◆--------------◆`, attachment: fs.createReadStream(__dirname + "/cache/joinMp4/hello.gif")} ,threadID));
	}
	else {
		try {
